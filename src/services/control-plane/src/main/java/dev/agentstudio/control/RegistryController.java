...
public class RegistryController {
    ...
    public RegistryController(JdbcClient jdbc, ObjectMapper json) { this.jdbc=jdbc; this.json=json; }
    ...
    List<ArtifactView> artifacts(@RequestHeader("X-Tenant-Id") String tenant,@PathVariable String registryId) {
        return jdbc.sql("select artifact_id,artifact_type,name,version,path,description,state,pulled_at from registry_artifacts where tenant_id=? and registry_id=? order by artifact_id")
          .params(required(tenant),registryId).query((rs,n)->new ArtifactView(rs.getString(1),RegistryType.valueOf(rs.getString(2)),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),rs.getTimestamp(8)==null?null:rs.getTimestamp(8).toInstant().toString())).list();
    }
    ...
    ArtifactView pull(@RequestHeader("X-Tenant-Id") String tenant,@PathVariable String registryId,@PathVariable String artifactId) {
        String t=required(tenant); RegistryView registry=find(t,registryId);
        ArtifactView a=jdbc.sql("select artifact_id,artifact_type,name,version,path,description,state,pulled_at from registry_artifacts where tenant_id=? and registry_id=? and artifact_id=? order by version desc limit 1")
          .params(t,registryId,artifactId).query((rs,n)->new ArtifactView(rs.getString(1),RegistryType.valueOf(rs.getString(2)),rs.getString(3),rs.getString(4),rs.getString(5),rs.getString(6),rs.getString(7),null)).single();
        ...
        return new ArtifactView(a.id(),a.type(),a.name(),a.version(),a.path(),a.description(),"PULLED",java.time.Instant.now().toString());
    }
    ...
    public record ArtifactView(String id,RegistryType type,String name,String version,String path,String description,String state,String pulledAt){}
}
