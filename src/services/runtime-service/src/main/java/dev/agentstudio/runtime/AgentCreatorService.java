...
public class AgentCreatorService {
    ...
    private static Set<String> tools(String text){Set<String> result=new LinkedHashSet<>();if(contains(text,"chat","conversation","follow-up","knowledge"))result.add("knowledge.search");if(contains(text,"postgres","postgresql")){result.add("postgres.describe-schema");result.add("postgres.query-readonly");}if(contains(text,"oracle")){result.add("oracle.describe-schema");result.add("oracle.query-readonly");}if(contains(text,"mysql")){result.add("mysql.describe-schema");result.add("mysql.query-readonly");}if(contains(text,"sql server","sqlserver")){result.add("sqlserver.describe-schema");result.add("sqlserver.query-readonly");}if(contains(text,"mongo","mongodb")){result.add("mongodb.describe-collections");result.add("mongodb.find-readonly");}if(contains(text,"redis")){result.add("redis.get");result.add("redis.search");}if(contains(text,"database","sql","jdbc")&&result.stream().noneMatch(value->value.contains("query"))){result.add("database.describe-schema");result.add("database.query-readonly");}if(contains(text,"headless","browser","javascript page","rendered page")){result.add("browser.navigate");result.add("browser.extract");if(contains(text,"click","fill","form","perform action","interact"))result.add("browser.act");}else if(contains(text,"website","web page","url")){result.add("web.fetch");result.add("web.extract");}if(contains(text,"http api","rest api","http request","api call"))result.add("http.request");if(contains(text,"chart","graph","visualize","plot"))result.add("chart.generate");if(contains(text,"email","mail")){result.add("email.draft");if(contains(text,"send","deliver","share"))result.add("email.send");}if(contains(text,"slack")){if(contains(text,"read","history","search","fetch"))result.add("slack.messages.read");if(contains(text,"send","post","notify","message","report","share"))result.add("slack.messages.send");}if(contains(text,"chat","conversation","follow-up","knowledge"))result.add("knowledge.store");return Collections.unmodifiableSet(result);}
